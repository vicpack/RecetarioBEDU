...
function areaAllcat() {
  if (this.readyState === 4 && this.status === 200) {
    ...
    let allCategories = data3[0][1].map(function (element) {
      //Here I´m moving all the areas from their object to an array
      return `${element.strCategory}`;
    });
    for (const i of allCategories) {
      //Here I created a lop to insert each area into the option list
...
function getSelectedValue2() {
  var selectedvalue = document.getElementById("searchBycategories").value;
...
function dishcat() {
  if (this.readyState === 4 && this.status === 200) {
    ...
    let allDishesByCategory = data4[0][1].map(function (element) {
      //Here I´m moving all the areas from their object to an array of objects
      return {
...
function getdishesbycategory(e) {
  ...
  xhrcatd.open(
    "GET",
    `${API_URL}/api/json/v1/1/filter.php?c=${e.target.value}`
  );
  xhrcatd.send();
}
