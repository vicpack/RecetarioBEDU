...
function getSelectedValue() {
  var selectedvalue = document.getElementById("searchByArea").value;
...
function dish() {
  if (this.readyState === 4 && this.status === 200) {
    ...
    let allDishesByAreas = data2[0][1].map(function (element) {
...
function getdishesbyarea(e) {
  ...
  xhraread.open(
    "GET",
    `${API_URL}/api/json/v1/1/filter.php?a=${e.target.value}`
  );
  xhraread.send();
}
